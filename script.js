...
// ----------
const takeSearchProducts = document.querySelector('#search-products');
const takeSearchBtn = document.querySelector('#search-btn');
// ----------
...
// -----------------
const PRODUCTS_API_BTN = 'https://api.mercadolibre.com/sites/MLB/search?q=';
// -----------------
...
// ------------------------------------------------------------------------------------------
// Meus aprimoramentos
takeSearchBtn.addEventListener('click', () => {
  return new Promise((resolve) => {
    fetch(`${PRODUCTS_API_BTN}${takeSearchProducts.value}`)
      .then((response) => response.json())
      .then((secondResponse) => secondResponse.results)
      .then((products) => {
        products.forEach((product) => {
